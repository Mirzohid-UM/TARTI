...
class CustomLogoutView(LogoutView):
    def dispatch(self, request, *args, **kwargs):
        # Foydalanuvchini tizimdan chiqishdan oldin biror amalni bajarish
        print(f"User {request.user.username} is logging out.")
        return super().dispatch(request, *args, **kwargs)

    def get_next_page(self):
        # Dinamik yo'naltirish
        return redirect('login') if self.next_page is None else self.next_page
...
def render_static_files(request):
    ...
    return render(request, 'HTML/show_static_files.html', {'static_files': yaml_content['static_files']})
